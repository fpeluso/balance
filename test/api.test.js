...
let server; let base; let file;
test.before(async () => { file = path.join(fs.mkdtempSync(path.join(os.tmpdir(), 'finance-')), 'db.json'); const store = new Store(file); server = http.createServer(createApp(store)); await new Promise((resolve) => server.listen(0, resolve)); base = `http://127.0.0.1:${server.address().port}`; });
test.after(async () => { await new Promise((resolve) => server.close(resolve)); fs.rmSync(path.dirname(file), { recursive: true, force: true }); });
